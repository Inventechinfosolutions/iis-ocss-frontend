...
import {
  buildBilingualValuesPatch,
  type BilingualValuesPatch,
} from "./applyExtracted"
import {
...
export function useDelayedClaimAutoFill(options: {
  applyBilingualPatch: (patch: BilingualValuesPatch) => void
  ...
  const run = async (file: File) => {
    ...
    try {
      ...
      const patch = buildBilingualValuesPatch(result.fields)
      setHasKannada(patch.hasKannada)
      options.applyBilingualPatch(patch)
      setStage("Complete")
      ...
      setError(
        message.includes("Failed to fetch")
          ? "Cannot reach Gemini. Check network or VITE_GEMINI_API_KEY in .env."
          : message,
      )
    } finally {
      setIsRunning(false)
