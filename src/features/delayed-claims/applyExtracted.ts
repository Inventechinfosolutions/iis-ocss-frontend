...
/**
 * Build a values patch from OCR results (by field id). Null/empty skipped.
 */
export function buildExtractedValuesPatch(
  ...
  for (const field of extracted) {
    ...
    const normalized = normalizeExtractedValue(field.id, String(field.value))
    if (!normalized) continue
    patch[field.id] = normalized
  }
