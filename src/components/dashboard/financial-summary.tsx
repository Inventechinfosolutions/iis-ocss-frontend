...
function MoneyHeroArt() {
  return (
    <div className="relative hidden h-[72px] w-[88px] shrink-0 sm:block" aria-hidden>
      <div className="absolute bottom-1 left-1 h-7 w-7 rounded-full bg-gradient-to-br from-[#93c5fd] to-[#3b82f6] shadow-md ring-2 ring-white/80" />
      <div className="absolute bottom-0 left-5 h-8 w-8 rounded-full bg-gradient-to-br from-[#bfdbfe] to-[#60a5fa] shadow-md ring-2 ring-white/80" />
      <div className="absolute right-0 top-0 flex size-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#dbeafe] to-[#93c5fd] shadow-lg ring-1 ring-[#3b82f6]/25">
        <div className="flex size-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#3b82f6] to-[#1d4ed8] text-white shadow-inner">
          <IndianRupee className="size-5" strokeWidth={2.5} />
        </div>
      </div>
      <Shield className="absolute -right-0.5 top-8 size-5 text-[#3b82f6]/70" />
    </div>
  )
}
...
export function FinancialSummary({ className }: { className?: string }) {
  return (
    <section
      ...
      className={cn("dashboard-panel stagger-in flex h-full flex-col p-4 sm:p-6", className)}
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Cash on hand, approved amounts, and fair share for victims
          </p>
        </div>
        <MoneyHeroArt />
      </div>
          ...
          return (
            <li key={metric.id}>
              ...
              >
                ...
                <p className="shrink-0 font-display text-sm font-semibold tabular-nums text-foreground sm:text-base">
                  {display}
                </p>
      ...
      <div className="mt-auto grid grid-cols-1 gap-2 rounded-2xl bg-slate-50 p-3 sm:grid-cols-3 dark:bg-muted/50">
